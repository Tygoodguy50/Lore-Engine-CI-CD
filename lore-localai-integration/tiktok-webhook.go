...
func main() {
	...
	go func() {
		log.Printf("🚀 Starting HTTP server on :80 for HTTP/SSL challenges...")
		if err := http.ListenAndServe(":80", nil); err != nil {
			log.Printf("⚠️ HTTP server on :80 failed: %v", err)
		}
	}()
	...
	log.Printf("🏥 Health URL: https://%s/health", domain)
	log.Printf("📊 Status URL: https://%s/api/webhooks/tiktok/status", domain)

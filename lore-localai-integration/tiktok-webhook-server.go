...
// TikTokWebhookServer handles TikTok webhook requests with HTTPS
...
func (s *TikTokWebhookServer) HandleTikTokWebhook(w http.ResponseWriter, r *http.Request) {
	...
	w.Header().Set("Access-Control-Allow-Origin", "https://www.tiktok.com")
...
func (s *TikTokWebhookServer) triggerViralAmplification(payload TikTokWebhookPayload, score float64) {
	...
	// Here you would integrate with your existing Discord/notification system
	// For example, call your lore dispatcher or send Discord notifications
...
func (s *TikTokWebhookServer) sendToDispatcher(data map[string]interface{}) {
	...
	// Example: POST to localhost:8084/lore/viral-amplification
	// You can implement the actual HTTP call here
}
...
func main() {
	...
	if webhookSecret == "" {
		log.Fatal("❌ TIKTOK_WEBHOOK_SECRET environment variable is required")
	}
	...
	certManager := autocert.Manager{
		...
		Cache:      autocert.DirCache("ssl-certs"), // Directory to store certificates
	}

	// Create HTTPS server with automatic SSL
	...
	// Create HTTP server for Let's Encrypt challenges
	server_http := &http.Server{
		Addr:    ":80",
		Handler: certManager.HTTPHandler(nil),
	}

	// Start HTTP server for SSL challenges
	go func() {
		log.Printf("🌐 Starting HTTP server on :80 for SSL challenges...")
		if err := server_http.ListenAndServe(); err != nil && err != http.ErrServerClosed {
	...
	log.Printf("📡 Webhook URL: https://%s/api/webhooks/tiktok", domain)
	log.Printf("💾 SSL certificates will be stored in: ./ssl-certs/")
	log.Printf("🎯 Server starting on :443 (HTTPS)")

	if err := server_https.ListenAndServeTLS("", ""); err != nil {

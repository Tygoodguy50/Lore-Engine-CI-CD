...
// LoreEvent represents a lore event for conflict detection
type LoreEvent struct {
	...
	LoreLevel         int                    `json:"LoreLevel"`
	Sentiment         float64                `json:"Sentiment"`
	CursedLevel       int                    `json:"CursedLevel"`
...
func testConflictAnalysisAPI(event LoreEvent) error {
	jsonData, err := json.Marshal(event)
	if err != nil {
		return err
	}

	resp, err := http.Post("http://localhost:8083/lore/conflicts/analyze", "application/json", bytes.NewBuffer(jsonData))
	if err != nil {
		return err
	}
...
func testConflictHistoryAPI() {
	...
	resp, err := http.Get("http://localhost:8083/lore/conflicts/history")
	if err != nil {
	...
	resp, err = http.Get("http://localhost:8083/lore/conflicts/stats")
	if err != nil {
...
func testHealthCheck() {
	resp, err := http.Get("http://localhost:8083/lore/conflicts/health")
	if err != nil {
...
func main() {
	...
	fmt.Println("⏳ Waiting for Conflict Detection server to be ready...")
